...
/**
 * Simple Game Status Component
 *
 * A simplified version of the GameStatus component that uses the SimpleGameRealtimeContext.
 */

import React, { useState, useEffect } from "react";
import { useSimpleGameRealtime } from "@/lib/contexts/SimpleGameRealtimeContext";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

export function GameStatusSimple() {
  const {
    isConnected,
    currentPlayer,
    gameInstance,
  ...
  return (
    <div className="p-4 border border-green-500 rounded-lg mb-4 bg-card">
      <h2 className="text-xl font-bold mb-4">Simple Game Connection</h2>
      ...
      {!currentPlayer ? (
        <div className="space-y-2 mb-4">
          ...
          <Button onClick={handleJoinGame} disabled={!gameId || joiningGame}>
            {joiningGame ? "Connecting..." : "Join Game (Simple)"}
          </Button>
        </div>
      ) : (
